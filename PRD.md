...
- 项目展示：
  - Sealos 自动化质量工具链：仓库记录可能包含私有内容，页面不直接放仓库链接，改为“私有仓库”。
  - Customer Advocate Agent：如果仓库仍为私有，页面不直接放仓库链接，改为“私有仓库”。
...
- 私有仓库只展示项目名称、问题背景、方法论和脱敏指标，不在公开页面暴露仓库地址。
...
- **私有仓库不可访问：** 部分仓库可能是私有或权限受限，页面只标注“私有仓库”。
